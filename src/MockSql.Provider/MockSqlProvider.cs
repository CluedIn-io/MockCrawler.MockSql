...
namespace CluedIn.Provider.MockSql
{
    public class MockSqlProvider : ProviderBase
    {
        private readonly IMockSqlClientFactory _MockSqlClientFactory;

        /**********************************************************************************************************
         * CONSTRUCTORS
         **********************************************************************************************************/

        public MockSqlProvider([NotNull] ApplicationContext appContext, IMockSqlClientFactory MockSqlClientFactory)
            : base(appContext, MockSqlConstants.CreateProviderMetadata())
        {
            _MockSqlClientFactory = MockSqlClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            var MockSqlCrawlJobData = new MockSqlCrawlJobData();
            if (configuration.ContainsKey(MockSqlConstants.KeyName.ConnectionString))
            { MockSqlCrawlJobData.ConnectionString = configuration[MockSqlConstants.KeyName.ConnectionString].ToString(); }


            return await Task.FromResult(MockSqlCrawlJobData);
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (jobData is MockSqlCrawlJobData MockSqlCrawlJobData)
            {
                ...
                dictionary.Add(MockSqlConstants.KeyName.ConnectionString, MockSqlCrawlJobData.ConnectionString);
            }
        ...
        public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var client = _MockSqlClientFactory.CreateNew(MockSqlCrawlJobData);
