...
{
  public class MockOrganisationsProvider : ProviderBase
  {
    private readonly IMockOrganisationsClientFactory _mockorganisationsClientFactory;

    /**********************************************************************************************************
     * CONSTRUCTORS
     **********************************************************************************************************/

    public MockOrganisationsProvider([NotNull] ApplicationContext appContext, IMockOrganisationsClientFactory mockorganisationsClientFactory)
        : base(appContext, MockOrganisationsConstants.CreateProviderMetadata())
    {
      _mockorganisationsClientFactory = mockorganisationsClientFactory;
    }
        ...
        Guid providerDefinitionId)
    {
      ...
      var mockorganisationsCrawlJobData = new MockOrganisationsCrawlJobData();
      if (configuration.ContainsKey(MockOrganisationsConstants.KeyName.ApiKey))
      { mockorganisationsCrawlJobData.ApiKey = configuration[MockOrganisationsConstants.KeyName.ApiKey].ToString(); }

      return await Task.FromResult(mockorganisationsCrawlJobData);
    }
        ...
        Guid providerDefinitionId)
    {
      ...
      if (jobData is MockOrganisationsCrawlJobData mockorganisationsCrawlJobData)
      {
        ...
        dictionary.Add(MockOrganisationsConstants.KeyName.ApiKey, mockorganisationsCrawlJobData.ApiKey);
      }
    ...
    public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
    {
      if (jobData == null) throw new ArgumentNullException(nameof(jobData));
            ...
            var client = _mockorganisationsClientFactory.CreateNew(mockorganisationsCrawlJobData);
